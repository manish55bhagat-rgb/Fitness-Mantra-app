...
export default async function handler(req, res) {
  ...
  if (msg.includes('401') || msg.includes('403') || lower.includes('api key') || lower.includes('permission')) {
    return res.status(403).json({ error: 'Gemini API key issue. Please create a new key in Google AI Studio and update GEMINI_API_KEY in Vercel.' });
  }

  if (msg.includes('429') || lower.includes('quota') || lower.includes('resource_exhausted')) {
    return res.status(429).json({ error: 'AI limit reached. Please try again later or use a fresh Gemini API key.' });
  }

  if (msg.includes('404') || lower.includes('not found') || lower.includes('model')) {
    return res.status(500).json({ error: 'AI model issue fixed in code, but this deployment still cannot access Gemini models. Redeploy once and keep only GEMINI_API_KEY in Vercel.' });
  }
